...
ivqr.gmm.rg <- function(Y, X.excl, Z.excl, D, tau, h=0, dB, max.time, upper, lower, weight.mtx, 
    ...
    #                      else{   W.hat <- weight.mtx}
      ...
      lmultiplier <- 1000000
      # one.vg <- matrix(cbind(1,0),ncol = 1)
      
      obj.fn <- function(b) n*ivqr.obj(b=b,  h=h.initial, weight.mtx=W.hat) + lmultiplier*abs(t(b)%*%one.vg-b.c)
