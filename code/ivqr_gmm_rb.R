...
ivqr.gmm.rb <- function(Y, X.excl, Z.excl, D, tau, h=0, dB, max.time, upper, lower, weight.mtx, 
    ...
    #                      else{   W.hat <- weight.mtx}
      ...
      ivqr.obj <- function(b, h, weight.mtx) {
          ...
          # lmultiplier <- 1000
          # one.v <- matrix(cbind(0,1),ncol = 1)
          
          obj.fn <- t(g.bar)%*% weight.mtx %*%g.bar 
          return(obj.fn)
      }
      lmultiplier <- 1000000
      # one.v <- matrix(cbind(0,1),ncol = 1) # log-linear, b.c <- log(b.init)
      # one.v <-  matrix(cbind(1,0),ncol = 1) # nonlinear, b.c <- b.init
      
      obj.fn <- function(b) n*ivqr.obj(b=b,  h=h.initial, weight.mtx=W.hat) + lmultiplier*abs(t(b)%*%one.vb-b.c)
      # obj.fn <- function(b) n*ivqr.obj(b=b,  h=h.initial, weight.mtx=W.hat) + lmultiplier*(t(b)%*%one.v-b.c)
     ...
     # J.stat <- obj.fn(b) - lmultiplier*abs(t(b)%*%one.v-b.c)
     J.stat <- n*ivqr.obj(b=b,  h=h.initial, weight.mtx=W.hat)
